...
#SBATCH -J gcn40
#SBATCH -o gcn40.%J.out
#SBATCH -e gcn40.%J.err
#SBATCH --array=1-20
...
epoch=${epoch_values[$(( trial % ${#epoch_values[@]} ))]}

python st_gcn_v2.py -traffic ${file} -gcn_type ${gcn} -lr ${lr} -depth ${depth} -loss ${loss} -epoch_size ${epoch} -ibex -close_size ${c} -period_size ${p} -trend_size ${t} -norm_type ${norm} -${fusion} -${transfer} -gcn_layer ${gcn_layer} -gate_type ${gate} -beta ${beta} -alpha ${alpha} -exp ${exp} -batch_size 128